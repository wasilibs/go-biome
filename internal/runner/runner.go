...
import (
	...
	"github.com/wasilibs/go-biome/v2/internal/wasm"
)
...
func Run(name string, args []string, stdin io.Reader, stdout io.Writer, stderr io.Writer, cwd string) int {
	...
	code, err := rt.CompileModule(ctx, wasm.Biome)
	if err != nil {
		panic(err)
	}
	...
	_, err = rt.NewHostModuleBuilder("wasi").NewFunctionBuilder().
		WithGoModuleFunction(api.GoModuleFunc(func(ctx context.Context, _ api.Module, stack []uint64) {
			...
			go func() {
				// TODO: Close after https://github.com/wazero/wazero/pull/2512
				// In practice, it's not that important since worker threads are long living, and the
				// internal state will be cleaned in WASM regardless, this just cleans up some globals / tables.
				// defer func() {
				// 	_ = child.Close(ctx)
				// }()
