...
go-biome is a distribution of [biome][1], that can be built with Go. It does not actually reimplement any
functionality of biome in Go, instead building it into a WebAssembly binary, and
...
Precompiled binaries are available in the [releases](https://github.com/wasilibs/go-biome/releases).
Alternatively, install the plugin you want using `go install`.

```bash
$ go install github.com/wasilibs/go-biome/cmd/biome@latest
```

To avoid installation entirely, it can be convenient to use `go run`

```bash
$ go run github.com/wasilibs/go-biome/cmd/biome@latest .
```
...
[1]: https://github.com/biomejs/biome
[2]: https://wazero.io/
